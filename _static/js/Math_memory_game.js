...
var Max_boxes = 3 // Number of boxes to solve to complete the game 
  ...
  ImgSource = image_source(); // Get the image source for the current box
    ...
    if (ImgFound == ImgSource.length*(BoxesSolved+1)/2) {
      if (BoxesSolved == Max_boxes-1) {
        $("#transitionMessage").text("You completed all the boxes and finished this round before the timer ran out! Congratulations! You earned maximum points. Wait for the timer to run out.");
        $("#transitionMessage").show();
      }
      else {
        BoxesSolved++;
